classdef gb_bidsobj
    ...
    methods
        ...
        function bidsobj = gb_bidsobj(init)
            ...
            bidsobj.redo_conversion = init.redo_conversion;
        ...
        function bidsobj = bids_conv_part(bidsobj)
            ...
            if bidsobj.s < 10
                su = sprintf('0%d', bidsobj.s);
            else
                su = sprintf('%d', bidsobj.s);
            end

            % Create directory
            subj_bids = fullfile(bidsobj.bids_dir, ['sub-', su]);
            mkdir(subj_bids);

            % Convert all series in subject folder to the BIDS directory
            subj_src = fullfile(bidsobj.src_dir_fMRI, bidsobj.subj_dir_fMRI);
            dicm2nii(subj_src, subj_bids);

            % T1 data
            % -------------------------------------------------------------------------

            % Create anatomy directory
            t1_bids = fullfile(subj_bids, 'anat');
            if bidsobj.redo_conversion && exist(t1_bids, 'dir')
                rmdir(t1_bids, 's');
            end
            if ~exist(t1_bids, 'dir')
                mkdir(t1_bids);
            end
            ...
            % Find all produced T1 NIfTIs in subj_bids
            t1_nifti = dir(fullfile(subj_bids, '*T1w*.nii*'));

            % load header data
            metadata = load(fullfile(subj_bids, 'dcmHeaders.mat'));
            fn = fieldnames(metadata.h);

            for i = 1:numel(t1_nifti)

                % pick the T1 field in order (matches the order of converted files)
                metadata_single = metadata.h.(fn{i});

                % convert units: date, elapsed time
                [bidsobj, metadata_un] = conv_unit(bidsobj, metadata_single);

                % remove/correct fields with identifiers
                [bidsobj, metadata_nid] = rem_ident(bidsobj, metadata_un);

                % save *T1w.json file
                json_file = fullfile(t1_bids, sprintf('sub-%s_T1w-%02d.json', su, i));
                savejson('', metadata_nid, json_file);

                % inform user
                fprintf('saving *T1w.json file %s\n', json_file);

                % clear variables
                clearvars metadata_single metadata_un metadata_nid

            end

            % Gather, move and rename converted files
            for i = 1:numel(t1_nifti)
                oldname = fullfile(subj_bids, t1_nifti(i).name);
                newname = fullfile(t1_bids, sprintf('sub-%s_T1w-%02d.nii.gz', su, i));
            ...
            epi_bids = fullfile(subj_bids, 'func');
            if bidsobj.redo_conversion && exist(epi_bids, 'dir')
                rmdir(epi_bids, 's');
            end
            if ~exist(epi_bids, 'dir')
                mkdir(epi_bids);
            end

            % Find all Predator EPIs
            epi_files = dir(fullfile(subj_bids, '**', 'ep2d_func_task_Predator_dir_AP_bold_s0*.nii*'));
            run_counter = 1;

            % Cycle over runs
            for f = 1:numel(epi_files)

                % Rename NIfTI
                oldname = fullfile(epi_files(f).folder, epi_files(f).name);
                newname = fullfile(epi_bids, sprintf('sub-%s_Predator-run-%02d_bold.nii.gz', su, run_counter));
                movefile(oldname, newname);

                % Load header data
                metadata = load(fullfile(epi_files(f).folder, 'dcmHeaders.mat'));
                meta = metadata.h.ep2d_func_task_Predator_dir_AP_bold_s014;

                if ~isfield(meta, 'SliceTiming')
                    error('Missing SliceTiming field in DICOM metadata for %s', newname);
                end

                % Needed for correct slice time for SPM (including ms -> s conversion)
                spm_sec = ((0.5 - meta.SliceTiming) * meta.RepetitionTime) / 1000;
                [~, spm_order] = sort(-meta.SliceTiming);
                meta.SliceTiming = spm_sec;
                meta.SliceOrder = spm_order;

                % Convert units, clean fields, add missing ones
                [~, meta_un] = conv_unit(bidsobj, meta);
                [~, meta_nid] = rem_ident(bidsobj, meta_un);
                [~, meta_comp] = add_fields(bidsobj, meta_nid);

                % Save *bold.json
                savejson('', meta_comp, strrep(newname, '.nii.gz', '.json'));

                run_counter = run_counter + 1;

                clearvars meta meta_un meta_nid meta_comp
            end

            % Declutter
            % -------------------------------------------------------------------------

            % Create declutter folder
            declutter_bids = fullfile(subj_bids, 'other');
            if bidsobj.redo_conversion && exist(declutter_bids, 'dir')
                rmdir(declutter_bids, 's');
            end
            if ~exist(declutter_bids, 'dir')
                mkdir(declutter_bids);
            end

            % Move all remaining files to other
            all_files = vertcat(...
                dir(fullfile(subj_bids, '*.nii*')), ...
                dir(fullfile(subj_bids, '*.json')), ...
                dir(fullfile(subj_bids, '*.mat')) ...
                );

            for i = 1:numel(all_files)
                oldname = fullfile(subj_bids, all_files(i).name);
                newname = fullfile(declutter_bids, all_files(i).name);
                movefile(oldname, newname);
            end
